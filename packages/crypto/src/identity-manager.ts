 ...
 * local so this module type-checks without the native bindings present; the module
 * is loaded lazily at runtime via a non-literal specifier (see below).
 */
 ...
 * type-checks and bundles even where the native bindings cannot be resolved at build
 * time (e.g. server/tooling contexts); resolution happens on first use.
 *
 ...
 *
 * TODO(phase-1): once `@signalapp/libsignal-client` native bindings are installed and
 * resolvable in each app, confirm this adapter against the pinned library version and
 * add an integration smoke test that a generated bundle round-trips through a session.
 */
